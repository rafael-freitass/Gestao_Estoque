...
class Tela_estoque(tk.Frame):
    ...
    def tabela_estoque(self):
        ...
        for product in lista_produtos:
            ...
            self.botao_adicionar = tk.Button(self.frame_botoes, text="+", command= lambda pid=produto_id: self.controller.registrar_entrada(pid), width=2, bg="#ffffff", borderwidth=1, relief="solid")
            self.botao_adicionar.grid(row=actual_row, column=5, padx=1, pady=1)

            self.botao_diminuir = tk.Button(self.frame_botoes, text="-", command= lambda pid=produto_id: self.controller.registrar_saida(pid), width=2, bg="#ffffff", borderwidth=1, relief="solid")
