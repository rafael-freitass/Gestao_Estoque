...
class TabelaProdutos(tk.Frame):
    todas_tabelas = []

    def __init__(self, parent, controller, tipo_tabela, callback_editar=None):
        ...
        TabelaProdutos.todas_tabelas.append(self)
    ...
    def montar_tabela(self):
        ...
        for product in lista_produtos:
            ...
            if self.tipo_tabela == "estoque":
                ...
                self.botao_editar = tk.Button(self.frame_botoes, text="Editar", command=lambda pid=produto_id: self.callback_editar(pid), width=2, bg="#ffffff", borderwidth=1, relief="solid")
    ...
    def atualizar_todas_tabelas(produto_id, nova_quantidade):
        for tabela in TabelaProdutos.todas_tabelas:
            label = tabela.label_quantidade.get(produto_id)
            if label:
    ...
    def adicionar_e_atualizar(self, produto_id):
        ...
        TabelaProdutos.atualizar_todas_tabelas(produto_id, nova_quantidade)
